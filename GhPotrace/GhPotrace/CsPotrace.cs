 ...
 public class dPoint
    {
        ...
        public dPoint()
        { }
        public static implicit operator Point2d(dPoint p)
        {
            return new Point2d(p.x, p.y);
        }
        public static implicit operator Point3d(dPoint p)
        {
   ...
   public class Potrace
   {
    #region Potrace classes and contants
       public static TurnPolicy turnpolicy = TurnPolicy.minority;
        ...
        public static bool ForcePolyline = false;
  ...
  static  double tangent(dPoint p0,dPoint p1,dPoint p2,dPoint p3,dPoint q0,dPoint q1) {
    ...
    #region Static function of Potrace
        ...
        static void ConvertBitmap(Bitmap bitmap,bool invert)
        {
            ...
            unsafe
            {
                ...
                for (int i = 0; i < l; i++)
                {
                  //  if ((0.2126 * (double)SourcePtr[4 * i + 2] + 0.7153 * (double)SourcePtr[4 * i + 1] + 0.0721 * (double)SourcePtr[4 * i]) < Treshold*255)
                        ...
                        Result[i] = Convert.ToByte(!invert);
                    else
                        Result[i] = Convert.ToByte(invert);
                }
            }
   ...
   double[] t = new double[om];
    ...
    path.curve = ocurve;
        ...
        static void tracetoList(DataTree<Curve> Curves)
        {
            ...
            for (int i = 0; i < pathlist.Count; i++) {
                ...
                for (int j = 0; j < P.curve.n; j++) {
                    if (P.curve.tag[j] == POTRACE_CORNER) {
                        Curve C = new PolylineCurve(new Point3d[] { LastPoint, P.curve.c[j * 3 + 1], P.curve.c[j * 3 + 2] });
                        if (C.IsValid) {
                            Curves.Add(C.ToNurbsCurve(), new Grasshopper.Kernel.Data.GH_Path(i));
                        }
                    }
                    else {
                        BezierCurve C = new BezierCurve(new Point3d[] { LastPoint, P.curve.c[j * 3], P.curve.c[j * 3 + 1], P.curve.c[j * 3 + 2] });
                        if (C.IsValid) {
                            Curves.Add(C.ToNurbsCurve(), new Grasshopper.Kernel.Data.GH_Path(i));
                        }
                    }
        ...
        static void tracetoList(List<Curve> Curves)
        {
            if (Curves == null) return;
            for (int i = 0; i < pathlist.Count; i++) {
                Path P = pathlist[i];
                ...
                for (int j = 0; j < P.curve.n; j++) {
                    ...
                    if (ForcePolyline || P.curve.tag[j] == POTRACE_CORNER) {
                        Curve C = new PolylineCurve(new Point3d[] { LastPoint, P.curve.c[j * 3 + 1], P.curve.c[j * 3 + 2] });
                        if (C.IsValid) {
                            cs.Add(C.ToNurbsCurve());
                        }
                    }
                    else {
                        BezierCurve C = new BezierCurve(new Point3d[] { LastPoint, P.curve.c[j * 3], P.curve.c[j * 3 + 1], P.curve.c[j * 3 + 2] });
                ...
                if (cs.Count > 0) {
                    // !expect only one closed region, may be buggy
                    Curve c = Curve.JoinCurves(cs)[0];
                    Curves.Add(c);
...
/*
 public static void Potrace_Trace(Bitmap Bitmap,  DataTree<Curve> crves)
 {
     ...
     tracetoList(crves);
        ...
        public static void Potrace_Trace(Bitmap Bitmap, DataTree<Curve> Curves, bool invert=false)
        {
            ConvertBitmap(Bitmap,invert);
            bmToPathlist();
            for (int i = 0; i < pathlist.Count; i++)
            {
                calcSums(pathlist[i]);
        ...
        public static void Potrace_Trace(Bitmap Bitmap, List<Curve> Curves, bool invert = false)
        {
            ConvertBitmap(Bitmap, invert);
            bmToPathlist();
            for (int i = 0; i < pathlist.Count; i++)
